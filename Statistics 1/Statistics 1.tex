...
\section{Notes on Hypothesis Testing}
...
$H_1$, let $\Phi$ be the $\mathcal{N}(0, 1)$ CDF, $\alpha$ be
our significance level:
\begin{align*}
    H_1 : \mu \neq \mu_0 & \Rightarrow
    C = (-\infty, -c^*] \cup [c^*, \infty)
    \tag{$ c^* = \Phi(1 - \frac{\alpha}{2}) $}              \\
...
For our observed test statistic $t_{obs}$, we want to generate a
critical region of values. This region $C$ changes based on our
$H_1$, let $T_k$ be the $t_k$ CDF, $\alpha$ be our significance level:
\begin{align*}
    H_1 : \mu \neq \mu_0 & \Rightarrow
    C = (-\infty, -c^*] \cup [c^*, \infty)
    \tag{$ c^* = T_{n-1}(1 - \frac{\alpha}{2}) $}              \\
    H_1 : \mu > \mu_0    & \Rightarrow
    C = [c^*, \infty) 
    \tag{$ c^* = T_{n-1}(1 - \alpha) $}      \\
    H_1 : \mu < \mu_0    & \Rightarrow 
...
For our observed test statistic $t_{obs}$, we want to generate a
critical region of values. This region $C$ changes based on our
$H_1$, let $T_k$ be the $t_k$ CDF, $\alpha$ be our significance
level:
\begin{align*}
    H_1 : \mu_X - \mu_Y \neq 0 & \Rightarrow
    C = (-\infty, -c^*] \cup [c^*, \infty)
    \tag{$ c^* = T_{n + m - 2}(1 - \frac{\alpha}{2}) $}           \\
    H_1 : \mu_X - \mu_Y > 0    & \Rightarrow
    C = [c^*, \infty) \tag{$ c^* = T_{n + m - 2}(1 - \alpha) $}   \\
    H_1 : \mu_X - \mu_Y < 0    & \Rightarrow C = (-\infty, -c^*].
    \tag{$ c^* = T_{n + m - 2}(1 - \alpha) $}
\end{align*}
