...
A hash table is a relation between keys and values.
It supports four operations:
...
\begin{center}
  \begin{tabular}{ || c | p{7.5cm} | c || }
    ...
    \hline
    \texttt{create()} & Creates a new hash table & $O(1)$ \\
    \hline
    \texttt{insert(k, v)} & Inserts the pair (\texttt{k, v}) overwriting if
    necessary & $O(1)$ \\
    \hline
...
\subsubsection{The implementation of hash tables}

We can generate a hash table by creating an array of some size $n$ where
each element in the array is a linked list. We use a \textbf{hash function}
to map data to a linked list. As we are using linked lists, if multiple 
keys map to the same index, we can just add them to the list - and when
looking up data, we can find the right list with the hash function and
then match our key.
\end{document}