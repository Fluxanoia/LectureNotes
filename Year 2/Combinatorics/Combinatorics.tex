...
of blocks $b$ can be defined as follows: \begin{gather*}
...
\subsection{Definition of a Walk}

We have that a walk of length $n$, is a set of
$n + 1$ vertices connected by $n$ edges.

\subsection{Definition of a Trail}

A trail is a walk where no edges are repeated.

\subsection{Definition of a Path}

A path is a walk where no vertices are repeated (barring the 
last one).

\subsection{Definition of a Cycle}

A cycle is a path whose last and first vertices are identical.

\subsection{Definition of Connected Graph}
