...
\end{gather*} We may represent the $k^{th}$ column vector of $A$ by 
...
\end{gather*} From this we can show that a matrix with all identical columns
must have zero determinant.

\subsubsection{Normalisation of the determinant}

For $A$ a square upper (or lower) triangular matrix, we have that
the determinant is the product of the diagonal entries.
Thus, the determinant of the identity is $1$.

\subsubsection{Determinants of the elementary matrices}
...
\subsubsection{Determinants of transposes}

We have that for a matrix $A$, the determinant of the transpose of $A$
is the same as $A$.
